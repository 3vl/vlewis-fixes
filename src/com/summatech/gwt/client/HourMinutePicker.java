...
public class HourMinutePicker extends Composite {
    ...
    private MouseOverHandler hourSelectedHandler = new MouseOverHandler() {
        ...
        public void onMouseOver(MouseOverEvent event) {
            closePopupOnBlur = false;
            Widget source = (Widget) event.getSource();
            refreshWidget((workingSuffix==null || "".equals(workingSuffix)) ? "AM" : workingSuffix, source.getElement()
                    .getPropertyInt("hour"), workingMinute == -1 ? 0 : workingMinute);
        }
    };
    ...
    private MouseOverHandler minuteSelectedHandler = new MouseOverHandler() {
        ...
        public void onMouseOver(MouseOverEvent event) {
            ...
            refreshWidget((workingSuffix==null || "".equals(workingSuffix)) ? "AM" : workingSuffix,
                    workingHour == -1 ? startHour : workingHour, source.getElement()
                            .getPropertyInt("minute"));
        }
    };
    ...
    public HourMinutePicker(PickerFormat format, int startHour, int endHour, int timeSlices) {
    ...
    private void createBody() {
        ...
        for (int i = 0; i < timeSlices; i++) {
            ...
            InlineLabel minuteLabel = createInlineLabel(NumberFormat.getFormat("00").format(min),
                    "timepickr-button");
        ...
        for (int i = (startHour < 12 ? startHour : 12); i < (endHour < 12 ? endHour : 12); i++) {
            InlineLabel hourLabelAM = createInlineLabel(NumberFormat.getFormat("00").format(i),
                    "timepickr-button");
        ...
        for (int i = (startHour < 12 ? 12 : startHour); i < (endHour < 12 ? 12 : endHour); i++) {
            InlineLabel hourLabelPM = createInlineLabel(
                    NumberFormat.getFormat("00").format(
                            PickerFormat._24_HOUR.equals(format) ? i : (i > 12 ? i % 12 : i)),
                    "timepickr-button");
    ...
    private void refreshWidget(String suffix, int hour, int minute) {
        ...
        DOM.setStyleAttribute(pnlHoursAM.getElement(), "left", 2
                - (pnlHoursAM.getOffsetWidth() / 2) - (pnlHoursAM.getOffsetWidth() % 2)
                + (amSuffixLabel.getOffsetWidth() / 2) + (amSuffixLabel.getOffsetWidth() % 2)
                + "px");

        DOM.setStyleAttribute(pnlHoursPM.getElement(), "left",
                pmSuffixLabel.getElement().getOffsetLeft() + (pmSuffixLabel.getOffsetWidth() / 2)
        ...
        if (workingHour != -1) {
            InlineLabel hourLabel = hourLabels.get(this.workingHour - startHour);
            DOM.setStyleAttribute(
                    pnlMinutes.getElement(),
                    "left",
                    hourLabel.getAbsoluteLeft() - lblInput.getAbsoluteLeft()
                            - (pnlMinutes.getOffsetWidth() / 2) + (hourLabel.getOffsetWidth() / 2)
                            + "px");
        } else {
            DOM.setStyleAttribute(pnlMinutes.getElement(), "left", 2
                    - (pnlMinutes.getOffsetWidth() / 2) - (pnlMinutes.getOffsetWidth() % 2)
                    + (amSuffixLabel.getOffsetWidth() / 2) + (amSuffixLabel.getOffsetWidth() % 2)
                    + "px");
        }
